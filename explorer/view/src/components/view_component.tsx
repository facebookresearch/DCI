...
function DisplayLayer({ baseImage, allMaskData, imgHeight, imgWidth, processedMasks, result }: 
  ...
  const baseCaption = result.short_caption;
  const finalCaption = result.extra_caption;
  const maskResults = result.mask_data;
...
function getFullCaption(submitData: any): string {
  let caption = submitData.short_caption.trim();
  caption += " " + submitData.extra_caption.trim();
  if (!(caption.endsWith('.') || caption.endsWith('!') || caption.endsWith('?'))) {
    caption = caption + '.';
  }
  for (let entryKey in submitData.mask_data) {
    let mask = submitData.mask_data[entryKey]
...
function DataView({ item }: {item: any}) {
  const maskData = item.mask_data;
  ...
  React.useEffect(() => {
    for (let maskKey in maskData) {
      ...
      convertMask(dat.outer_mask, imgWidth, imgHeight, (newMask) => {
        setProcessedMasks((prevMasks: any) => {return {...prevMasks, [maskKey]: newMask}})})
    }
    setMasksLoaded(true);
  ...
  const wordCount = currentWordCount(item);
